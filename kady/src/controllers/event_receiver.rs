use crate::{ensure_user_db_presence as ensure_user, Context};
        ...
        Event::InteractionCreate(interaction) => {
            match interaction.kind {
                ...
                InteractionType::ApplicationCommand => {
                    ...
                    if let InteractionData::ApplicationCommand(command_data) =
...
async fn verify_user_tos_acceptance(ctx: &Context, user_id: &Id<UserMarker>) -> bool {
    let query = sqlx::query!(
        "SELECT accepted_tos FROM users WHERE id=?;",
        user_id.to_string()
    )
